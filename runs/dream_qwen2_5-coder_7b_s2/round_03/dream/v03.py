...
new branches opened in each round to improve focus and reduce redundancy. Added
a more nuanced approach to deciding when to open new branches.
"""
...
class Policy:
    ...
    def select_batch(self, view: View) -> list[int]:
        ...
        deepening = deepening[:view.W // 3]

        # Repair recoverable failures
        recoverable = [b for b in branches if frontier_scores[b] is not None and b not in deepening]
        recoverable = recoverable[:view.W // 3]

        # Open new branches if there's room and the best score has improved
        new_branches = [view.root] * min(view.W // 3, view.root_slots())
        if view.best_score() > -2.175:
            new_branches = new_branches[:view.W // 3]
